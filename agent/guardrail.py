...
def run_guarded_audit(
    ...
    checks = [
        {
            ...
            "label": "数据来源可用于生产",
            "detail": "当前是明确标记的合成演示数据，不能支持交易决策。" if dataset_kind != "REAL_MARKET_DATA" else "数据来源标记为真实市场数据。",
        },
        {
            "code": "DATA_QUALITY",
            "status": "BLOCKED" if data_quality.get("errors") else "PASSED",
            "label": "数据质量检查",
            "detail": f"{len(data_quality.get('errors') or [])} 个错误，{len(data_quality.get('warnings') or [])} 个警告。",
        },
        {
            "code": "WALK_FORWARD",
            "status": "PASSED" if len(walk_forward.get("folds") or []) >= 3 else "BLOCKED",
            "label": "Walk-forward 样本外验证",
            "detail": f"已运行 {len(walk_forward.get('folds') or [])} 个滚动窗口。",
        },
        {
            "code": "PRODUCTION_READINESS",
            "status": "PASSED" if bool(backtest.get("production_eligible")) else "BLOCKED",
            "label": "生产就绪条件",
            "detail": "；".join(_translate_reason(reason) for reason in reasons) or "引擎未返回生产准入结论。",
        },
    ]
    ...
    trace = [
        ...
        },
        {
            ...
            "status": "COMPLETED",
            "title": "检查数据来源与指纹",
            "summary": f"识别为 {dataset_kind}，共 {int((backtest.get('period') or {}).get('sessions') or 0)} 个交易日。",
        ...
        },
        {
            ...
            "status": "COMPLETED",
            "title": "运行策略与三条基线",
            "summary": "回测引擎已计算全样本、样本外表现、成本和基准差异。",
        ...
        },
        {
            ...
            "tool": "audit_backtest",
            "status": "COMPLETED",
            "title": "执行样本外与 Walk-forward 审计",
            "summary": f"样本外年化收益 {_percent(out_of_sample.get('annualized_return'))}，但不据此推断未来收益。",
            "evidence": {
        ...
        },
        {
            ...
            "status": decision,
            "title": "确定性风险门禁做最终裁决",
            "summary": "证据不足，拒绝进入模拟交易。" if blocked else "全部硬性条件通过，可进入人工复核。",
        ...
        },
        {
            ...
            "title": "订单意图未创建" if blocked else "等待人工批准后创建订单意图",
            "summary": "风险门禁已阻断执行路径，数据库未写入订单意图。" if blocked else "Agent 无权绕过人工批准。",
    ...
    return {
        ...
        "task": task,
        "verdict": decision,
        "headline": "这笔交易，我拒绝执行。" if blocked else "证据通过，等待你的最终判断。",
        "summary": "回测看起来不错，但数据来源和生产验证条件不够可信。" if blocked else "硬性门禁通过，但系统仍不会自动下单。",
        ...
        "disclaimer": "仅用于研究流程演示，不构成投资建议，不代表真实收益。",
    }
...
def _percent(value: object) -> str:
    ...
    return "未知" if number is None else f"{number:.2%}"
