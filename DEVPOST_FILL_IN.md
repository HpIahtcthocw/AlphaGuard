# AlphaGuard — Devpost Fill-In (copy-paste ready)
...
> AlphaGuard is the agent-facing surface of an investment research system: under WebMCP it hands agents **real, deterministic risk-gate tools** instead of a screen to click. The agent calls, the gate vetoes, and a `BLOCKED` verdict is final and machine-verifiable. It's a truth-server for the new agent interface — built to refuse what it can't prove.
...
| Public Code Repo | `https://github.com/HpIahtcthocw/AlphaGuard` |
...
2. **代码必须在默认分支** → 现在 AlphaGuard 只在 `feat/goai-alpha-guard-agent` 分支，而评委打开仓库看到的是默认分支（main）。**两条路二选一**：把分支合并推到 main，或在 GitHub 设置里把默认分支改成 feature 分支。**不处理评审会看到旧的基线代码（没 AlphaGuard）。**
